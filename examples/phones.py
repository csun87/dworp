...
class Person(dworp.Agent):
    ...
    def step(self, now, env):
        ...
        for i in range(0, len(neighbors)):
            ...
            if nstate[0] == 1 and nstate[4] >= 0.5:
                num_apple += 1
            elif nstate[0] == 2 and nstate[4] >= 0.5:
                num_android += 1
            elif nstate[0] == 0 and nstate[4] >= 0.5:
                num_none += 1
        ...
        if np.random.random() < 0.1: # 10% chance of randomly switching (which some people decide to do)
                self.state[0] = np.random.random_integers(1, 2)
...
class PyGameRenderer(dworp.Observer):
    ...
    def draw(self, agents):
        ...
        for agent in agents:
            ...
            if agent.state[0] == 0:  # no phone
                color = (0, 191, 255) # blue
            else:
                if agent.state[0] == 1:  # apple
